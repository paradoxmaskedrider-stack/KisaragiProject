...
public class BackpackPocketMenu extends AbstractContainerMenu {

    private final ItemStack backpackStack;
    private final BackpackInventory backpackInventory;

    public BackpackPocketMenu(int id, Inventory playerInventory, ItemStack backpackStack) {
        super(ModMenus.BACKPACK_POCKET.get(), id);
        ...
        int pocketSlots = Math.min(storageSize, 9);

        final int pocketStartX = 9;
        final int pocketY = 61;

        for (int col = 0; col < pocketSlots; col++) {
            this.addSlot(new Slot(backpackInventory, col, pocketStartX + col * 18, pocketY));
        }

        final int invStartX = 9;
        final int invStartY = 115;
